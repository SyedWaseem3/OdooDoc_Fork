...
customers, and customize the website to support B2B, B2C, or both business models.
...
only their products instead. To configure a B2B-specific ecommerce shop:

#. Go to :menuselection:`Website --> Configuration --> Settings`, scroll down to :guilabel:`Shop
   - Products`, and under :guilabel:`Display Product Prices` select :guilabel:`Tax-Excluded`.
#. In that same section, enable the :ref:`Prevent Sale of Zero Priced Product
   <ecommerce/prices/hide-prices>` feature.

   .. note::
      The redirect link and the corresponding button can be :ref:`customized
      <ecommerce/checkout/prevent-sale>`.

#. Configure a :ref:`pricelist <ecommerce/prices/pricelists>` with all prices set to `0.00`.
...
<ecommerce/customer_accounts/account-creation>` setting option to :guilabel:`On inviation`.
...
.. tip::
   Use tags to quickly identify customers who have submitted the form. To do so, add
   a :icon:`fa-plus` :guilabel:`Field` to the form, and under :guilabel:`Type`, select
   :guilabel:`Tags`. Remove any other unnecessary tags by clicking :icon:`fa-minus` under the
   :guilabel:`Checkbox List`. Toggle only the tags related to B2B/B2C account requests to
   automatically check the buttons and assign the tags when a customer fills in the form. Set the
   :guilabel:`Visibility` to :guilabel:`hidden`.
